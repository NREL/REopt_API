...
def validate_RunOutput_for_outage_simulator(ro):
    ...
    if ro.pv_kw > 0 and ro.pv_kw_ac_hourly is None:
        raise ValidationError('Scenario PV production undefined (with pv_kw > 0)')
    ...
    if ro.batt_kw > 0 and ro.batt_efficiency is None:
        raise ValidationError('Scenario battery efficiency undefined (with batt_kw > 0)')

    if ro.batt_kw > 0 and ro.batt_inverter_efficiency is None:
        raise ValidationError('Scenario battery inverter efficiency undefined (with batt_kw > 0)')

    if ro.batt_kw > 0 and ro.batt_rectifier_efficiency is None:
        raise ValidationError('Scenario battery rectifier efficiency undefined (with batt_kw > 0)')
