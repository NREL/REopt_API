...
def site_summary_df(site):
    rows = ["PV size (kW)",
            ...
            "Storage size (kWh)",
            "Generator size (kW)",
            "Year 1 PV energy consumed (MWh)",
            "Year 1 PV energy curtailed (MWh)",
            "Year 1 grid energy consumed (MWh)",
            "Year 1 Optimal energy charges ($000)",
            "Year 1 Optimal demand charges ($000)",
            "Year 1 energy charge savings ($000)",
            "Year 1 demand charge savings ($000)",
            # "Energy charge savings ($000)",
            # "Demand charge savings ($000)",
            # "PV PPA charges ($000)",
            "Annualized payment to third party ($000)",
            "Capital costs ($000)",
            "Year 1 O&M costs ($000)",
            "LCC ($000)",
            "NPV ($000)",
    ...
    for scen_abrv,col_title in scenarios.items():
        ...
        if "Business as Usual" not in df.columns and add_BAU_col:
            df["Business as Usual"] = [ "-",
                                        ...
                                        results["ElectricTariff"]["year_one_energy_cost_bau_us_dollars"]/1000,
                                        results["ElectricTariff"]["year_one_demand_cost_bau_us_dollars"]/1000,
                                        "-",
                                        "-",
                                        # "-",
                                        # "-",
                                        ...
                                        "-",
                                        "-",
                                        (results["Generator"]["existing_gen_year_one_variable_om_cost_us_dollars"]+results["PV"]["existing_pv_om_cost_us_dollars"])/1000 if results["Generator"]["existing_gen_year_one_variable_om_cost_us_dollars"]+results["PV"]["existing_pv_om_cost_us_dollars"] != 0 else "-",
        ...
        df[col_title] = [results["PV"]["size_kw"] if results["PV"]["size_kw"] != 0 else "-",
                        ...
                        (sum(results["PV"]["year_one_power_production_series_kw"])-sum(results["PV"]["year_one_curtailed_production_series_kw"]))/1000 if results["PV"]["year_one_energy_produced_kwh"] != 0 else "-",
                        sum(results["PV"]["year_one_curtailed_production_series_kw"])/1000,
                        results["ElectricTariff"]["year_one_energy_supplied_kwh"]/1000,
                        results["ElectricTariff"]["year_one_energy_cost_us_dollars"]/1000,
                        results["ElectricTariff"]["year_one_demand_cost_us_dollars"]/1000,
                        (results["ElectricTariff"]["year_one_energy_cost_bau_us_dollars"]-results["ElectricTariff"]["year_one_energy_cost_us_dollars"])/1000,
                        (results["ElectricTariff"]["year_one_demand_cost_bau_us_dollars"]-results["ElectricTariff"]["year_one_demand_cost_us_dollars"])/1000,
                        ...
                        results["Financial"]["year_one_om_costs_us_dollars"]/1000 if results["Financial"]["year_one_om_costs_us_dollars"] != 0 else "-",
                        results["Financial"]["lcc_us_dollars"]/1000,
                        results["Financial"]["npv_us_dollars"]/1000 if results["Financial"]["npv_us_dollars"] != 0 else "-",
                        results["Financial"]["simple_payback_years"] if results["Financial"]["simple_payback_years"] != None else "-",
                        "",
