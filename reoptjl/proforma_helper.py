...
def proforma_helper(results:dict, output):
    ...
    def optimal_system_design(start_row, shift):
        ...
        if 'PV' in results['outputs'].keys():
            lcoe_formula = '=ROUND(((PV_purchased_installed_costs + -1*(NPV(Offtaker_discount_rate/100, Purchased_PV_OM_series_cost)) - SUM(PV_total_CBI, PV_total_IBI, NPV(Offtaker_discount_rate/100,Purchased_PV_PBI_combined), NPV(Offtaker_discount_rate/100,PV_itc_amount), NPV(Offtaker_discount_rate/100,Purchased_PV_income_tax_savings))) ) / ((NPV(Offtaker_discount_rate/100, Total_PV_annual_gen_kwh_range) - NPV(Offtaker_discount_rate/100,Existing_PV_annual_gen_kwh_range))), 4)'
        else:
            lcoe_formula = '=0.0'
        ...
        if 'Wind' in results['outputs'].keys():
            lcoe_formula = '=ROUND((Wind_installed_costs + (-1 * NPV(Offtaker_discount_rate/100,Wind_OM_series_cost)) - SUM(Wind_total_CBI ,Wind_total_IBI ,NPV(Offtaker_discount_rate/100,Wind_PBI_combined), NPV(Offtaker_discount_rate/100,\'Optimal Cash Flow\'!C145),NPV(Offtaker_discount_rate/100, Wind_income_tax_savings)))/ NPV(Offtaker_discount_rate/100, Wind_annual_gen_kwh_range),4)'
        else:
            lcoe_formula = '=0.0'
    ...
    def system_costs(start_row, shift):
        ...
        shift = add_inputs_outputs_defined_cell(start_row, shift, 'ASHP Space Heater Installed Cost ($)', space_ashp_capex, 'text_format', 'ASHPSpaceheater_installed_costs')
        shift = add_inputs_outputs_defined_cell(start_row, shift, 'ASHP Water Heater Installed Cost ($)', water_ashp_capex, 'text_format', 'ASHPWaterheater_installed_costs')
    ...
    def add_tech_depreciation_info(section_end):
        depr_row = ['DEPRECIATION', 'PV', 'BATTERY', 'WIND', 'CHP', 'Absorption Chiller', 'ASHP Space Heater', 'ASHP Water Heater', 'Hot TES', 'Cold TES', 'SteamTurbine', 'GHP']
        section_end += 1
    ...
    def add_macrs_years_for_techs(annual_value_rows, macrs_option_year_row):
        ...
        proforma.define_name('Battery_MACRS_years', '=\'Inputs and Outputs\'!$C$'+str(macrs_option_year_row))
        proforma.define_name('Wind_MACRS_years', '=\'Inputs and Outputs\'!$D$'+str(macrs_option_year_row))
        ...
        techs_eligible_for_macrs = [
            'PV_MACRS_years','Battery_MACRS_years','Wind_MACRS_years','CHP_MACRS_years',
    ...
    def add_bau_direct_own_sheet():
        ...
        baucashf_sheet.write_dynamic_array_formula('C10:AA10', '=-Diesel_BAU_fuel_used_cost * (1 + OM_cost_escal_pct/100)^C2:AA2', o['text_format'])
    ...
    ## OPERATING EXPENSES
    def add_operating_expense_rows(row, third_party_own):
        ...
        if third_party_own:
            proforma.define_name('Purchased_PV_OM_series_cost', '=\'Third-party Owner Cash Flow\'!$C$'+str(row-1)+':\'Third-party Owner Cash Flow\'!$AA$'+str(row-1))
        else:
            proforma.define_name('Purchased_PV_OM_series_cost', '=\'Optimal Cash Flow\'!C'+str(row-1)+':\'Optimal Cash Flow\'!AA'+str(row-1))
        ...
        if third_party_own:
            proforma.define_name('Wind_OM_series_cost', '=\'Third-party Owner Cash Flow\'!C'+str(row-1)+':\'Third-party Owner Cash Flow\'!AA'+str(row-1))
        else:
            proforma.define_name('Wind_OM_series_cost', '=\'Optimal Cash Flow\'!C'+str(row-1)+':\'Optimal Cash Flow\'!AA'+str(row-1))
        ...
        if not third_party_own:
            row = add_operating_expenses(row, 'Generator diesel fuel cost ($)', '=-Diesel_fuel_used_cost * (1+OM_cost_escal_pct/100)^')
    ...
    def add_direct_cash_incentives(row):
        ...
        if third_party_own:
            proforma.define_name('PV_total_IBI', '=\'Third-party Owner Cash Flow\'!$B$'+str(row))
        else:
            proforma.define_name('PV_total_IBI', '=\'Optimal Cash Flow\'!$B$'+str(row))
        row += 1
        ...
        if third_party_own:
            proforma.define_name('PV_total_CBI', '=\'Third-party Owner Cash Flow\'!$B$'+str(row))
        else:
            proforma.define_name('PV_total_CBI', '=\'Optimal Cash Flow\'!$B$'+str(row))
        ...
        optcashf_sheet.write_row('A'+str(row), [' Total', '=SUM(B'+str(row-2)+', B'+str(row-1)+')'], o['text_format'])
        if third_party_own:
            proforma.define_name('Wind_total_IBI', '=\'Third-party Owner Cash Flow\'!$B$'+str(row))
        else:
            proforma.define_name('Wind_total_IBI', '=\'Optimal Cash Flow\'!$B$'+str(row))
        row += 1
        ...
        optcashf_sheet.write_row('A'+str(row), [' Total', '=SUM(B'+str(row-3)+':B'+str(row-1)+')'], o['text_format'])
        if third_party_own:
            proforma.define_name('Wind_total_CBI', '=\'Third-party Owner Cash Flow\'!$B$'+str(row))
        else:
            proforma.define_name('Wind_total_CBI', '=\'Optimal Cash Flow\'!$B$'+str(row))
        ...
        if third_party_own:
            proforma.define_name('Purchased_PV_PBI_combined', '=\'Third-party Owner Cash Flow\'!C'+str(row)+':\'Third-party Owner Cash Flow\'!AA'+str(row))
        else:
            proforma.define_name('Purchased_PV_PBI_combined', '=\'Optimal Cash Flow\'!C'+str(row)+':\'Optimal Cash Flow\'!AA'+str(row))
        row += 1
        ...
        if third_party_own:
            proforma.define_name('Wind_PBI_combined', '=\'Third-party Owner Cash Flow\'!C'+str(row)+':\'Third-party Owner Cash Flow\'!AA'+str(row))
        else:
            proforma.define_name('Wind_PBI_combined', '=\'Optimal Cash Flow\'!C'+str(row)+':\'Optimal Cash Flow\'!AA'+str(row))
        row += 1
        ...
        if third_party_own:
            proforma.define_name('Sum_taxable_cash_incentives', '=\'Third-party Owner Cash Flow\'!$B$'+str(row-1)+':$AA$'+str(row-1))
            proforma.define_name('Sum_nontaxable_cash_incentives', '=\'Third-party Owner Cash Flow\'!$B$'+str(row)+':$AA$'+str(row))
        else:
            proforma.define_name('Sum_taxable_cash_incentives', '=\'Optimal Cash Flow\'!$B$'+str(row-1)+':$AA$'+str(row-1))
            proforma.define_name('Sum_nontaxable_cash_incentives', '=\'Optimal Cash Flow\'!$B$'+str(row)+':$AA$'+str(row))

        return row

    ## CAPITAL DEPRECIATION
    def add_capital_depreciation_LCOE_calculations(row):
        row += 2
        ...
        for idx in range(2, 27):
            ...
            optcashf_sheet.write_formula(
                col+str(row),
                '=SUM( '+col+'95, '+col+'100, '+col+'105, '+col+'110, '+col+'115, '+col+'120, '+col+'125, '+col+'130, '+col+'135, '+col+'140, '+col+'145)',
                o['text_format_2']
            )
        if third_party_own:
            proforma.define_name('Total_depreciation_shield', '=\'Third-party Owner Cash Flow\'!$C$'+str(row)+':$AA$'+str(row))
        else:
            proforma.define_name('Total_depreciation_shield', '=\'Optimal Cash Flow\'!$C$'+str(row)+':$AA$'+str(row))
        row += 2
        ...
        optcashf_sheet.write_dynamic_array_formula('C'+str(row)+':AA'+str(row)+'', '=((-1 * C10:AA10) + C95:AA95) * Offtaker_tax_rate/100', o['text_format_2'])
        proforma.define_name('Purchased_PV_income_tax_savings', '=\'Optimal Cash Flow\'!C'+str(row)+':\'Optimal Cash Flow\'!AA'+str(row))
        row += 1
        optcashf_sheet.write('A'+str(row), 'Wind income tax savings, $', o['text_format_2'])
        optcashf_sheet.write_dynamic_array_formula('C'+str(row)+':AA'+str(row)+'', '=((-1 * C12:AA12) + C100:AA100) * Offtaker_tax_rate/100', o['text_format_2'])
        proforma.define_name('Wind_income_tax_savings', '=\'Optimal Cash Flow\'!C'+str(row)+':\'Optimal Cash Flow\'!$AA$'+str(row))
        return row + 2

    ## FEDERAL ITC CALCULATIONS
    def add_federal_itc_calculations(row, third_party_own):
        ...
        if third_party_own:
            proforma.define_name('PV_itc_basis', '=\'Third-party Owner Cash Flow\'!$B$'+str(row))
        else:
            proforma.define_name('PV_itc_basis', '=\'Optimal Cash Flow\'!$B$'+str(row))
        row += 1
        ...
        if third_party_own:
            proforma.define_name('PV_itc_amount', '=\'Third-party Owner Cash Flow\'!$C$'+str(row))
        else:
            proforma.define_name('PV_itc_amount', '=\'Optimal Cash Flow\'!$C$'+str(row))
        row += 1
        ...
        if third_party_own:
            proforma.define_name('Wind_itc_basis', '=\'Third-party Owner Cash Flow\'!$B$'+str(row))
        else:
            proforma.define_name('Wind_itc_basis', '=\'Optimal Cash Flow\'!$B$'+str(row))
        row += 1
        ...
        if third_party_own:
            proforma.define_name('Wind_itc_amount', '=\'Third-party Owner Cash Flow\'!$C$'+str(row))
        else:
            proforma.define_name('Wind_itc_amount', '=\'Optimal Cash Flow\'!$C$'+str(row))
        row += 1
        ...
        if third_party_own:
            proforma.define_name('Battery_itc_basis', '=\'Third-party Owner Cash Flow\'!$B$'+str(row))
        else:
            proforma.define_name('Battery_itc_basis', '=\'Optimal Cash Flow\'!$B$'+str(row))
        row += 1
        ...
        if third_party_own:
            proforma.define_name('CHP_itc_basis', '=\'Third-party Owner Cash Flow\'!$B$'+str(row))
        else:
            proforma.define_name('CHP_itc_basis', '=\'Optimal Cash Flow\'!$B$'+str(row))
        row += 1
        ...
        if third_party_own:
            proforma.define_name('GHP_itc_basis', '=\'Third-party Owner Cash Flow\'!$B$'+str(row))
        else:
            proforma.define_name('GHP_itc_basis', '=\'Optimal Cash Flow\'!$B$'+str(row))
        row += 1
        ...
        if third_party_own:
            proforma.define_name('Total_federal_itc', '=\'Third-party Owner Cash Flow\'!$C$'+str(row)+':$AA$'+str(row))
        else:
            proforma.define_name('Total_federal_itc', '=\'Optimal Cash Flow\'!$C$'+str(row)+':$AA$'+str(row))

        return row + 2

    ## TOTAL CASH FLOW CALCULATIONS
    def add_total_cash_flow_calculations(row):
        ...
        optcashf_sheet.write_formula('B'+str(row), '=-GHX_residual_value', o['text_format'])
        row+=1
        optcashf_sheet.write('A'+str(row), 'Operating expenses, after-tax', o['text_format'])
        optcashf_sheet.write_dynamic_array_formula('C'+str(row)+':AA'+str(row), '=(Total_operating_expenses - Txded_operating_expenses) + Txded_operating_expenses * (1 - Offtaker_tax_rate/100)', o['text_format'])
        row+=1
        optcashf_sheet.write('A'+str(row), 'Total Cash incentives, after-tax', o['text_format'])
        optcashf_sheet.write_dynamic_array_formula('B'+str(row)+':AA'+str(row), '=(Sum_taxable_cash_incentives * (1 - Offtaker_tax_rate/100)) + Sum_nontaxable_cash_incentives', o['text_format'])
        row+=1
        optcashf_sheet.write('A'+str(row), 'Depreciation Tax Shield', o['text_format'])
        optcashf_sheet.write_dynamic_array_formula('C'+str(row)+':AA'+str(row), '=Total_depreciation_shield * Offtaker_tax_rate/100', o['text_format'])
        row+=1
        ...
        proforma.define_name('OPT_free_cash_flow_range', '=\'Optimal Cash Flow\'!$B$'+str(row)+':$AA$'+str(row))
        row+=1
        ...
        optcashf_sheet.write_dynamic_array_formula('C'+str(row)+':AA'+str(row), '=C'+str(row-1)+':AA'+str(row-1) +' / (1 + Offtaker_discount_rate/100)^C2:AA2', o['text_format_2'])
        row+=1
    ...
    ## PROJECT ECONOMICS
    def add_project_economics_summary():
    ...
    start_row = 3
    shift = 0 # quickly shift local rows

    section_end = optimal_system_design(start_row, shift)
    ...
    annual_values_row_labels = [
        ...
        ['Free Cash Flow', None],
        ['Cumulative Free Cash Flow', None]
    ]
    ...
    row = 5
    row, Battery_kw_replacement_row, Battery_kwh_replacement_row = add_operating_expense_rows(row, third_party_own)
    ...
    optcashf_sheet.write('A'+str(Battery_kwh_replacement_row), 'Battery kW Replacement Cost', o['text_format'])
    ...
    for idx in range(2, 27):
        ...
        optcashf_sheet.write_formula(col+str(row+1), '=IF(Offtaker_tax_rate > 0, '+col+str(row-1)+', 0)', o['text_format_2'])
    if third_party_own:
        proforma.define_name('Total_operating_expenses', '=\'Third-party Owner Cash Flow\'!$C$'+str(row)+':$AA$'+str(row))
        proforma.define_name('Txded_operating_expenses', '=\'Third-party Owner Cash Flow\'!$C$'+str(row+1)+':$AA$'+str(row+1))
    else:
        proforma.define_name('Total_operating_expenses', '=\'Optimal Cash Flow\'!$C$'+str(row)+':$AA$'+str(row))
        proforma.define_name('Txded_operating_expenses', '=\'Optimal Cash Flow\'!$C$'+str(row+1)+':$AA$'+str(row+1))

    row += 3
    row = add_direct_cash_incentives(row)
    row = add_capital_depreciation_LCOE_calculations(row)
    row = add_federal_itc_calculations(row, third_party_own)
    row = add_total_cash_flow_calculations(row)

    def add_third_party_cash_flow_calculations(row):
    ...
    if third_party_own:
        add_third_party_cash_flow_calculations(row)

    ## SUMMARY PROJECT ECONOMICS
    if not third_party_own:
        add_project_economics_summary()
    ...
    def add_host_cash_flow_thirdpartyown():
        ...
        baucashf_sheet.write('A14', 'Optimal Free Cash Flow', o['text_format_2'])
        ...
        baucashf_sheet.write_dynamic_array_formula('C11:AA11', '=-Diesel_BAU_fuel_used_cost * (1 + OM_cost_escal_pct/100)^C2:AA2 * Generator_existing_kw', o['text_format'])
        baucashf_sheet.write_dynamic_array_formula('C12:AA12', '=-Diesel_fuel_used_cost * (1 + OM_cost_escal_pct/100)^C2:AA2 * Generator_existing_kw', o['text_format'])
        baucashf_sheet.write_array_formula('C13:AA13', '{=C3:AA3 + C4:AA4 + C11:AA11}', o['text_format_2'])
        baucashf_sheet.write_array_formula('C14:AA14', '{=C5:AA5 + C6:AA6 + C10:AA10 + C12:AA12}', o['text_format_2'])
        baucashf_sheet.write_array_formula('C15:AA15', '{=-C3:AA3 - C4:AA4 + C5:AA5 + C6:AA6 + C10:AA10 - C11:AA11 + C12:AA12 - C7:AA7 + C8:AA8 + C9:AA9}', o['text_format_2'])
        baucashf_sheet.write_array_formula('C16:AA16', '{=IF(Offtaker_tax_rate/100 > 0, C15:AA15, 0)}', o['text_format_2'])

        baucashf_sheet.write_row('A18', ['Total Cash Flows']+[None]*26, o['subheader_format'])
        baucashf_sheet.write('A19', 'Net Operating expense, after tax', o['text_format_2'])
        baucashf_sheet.write_array_formula('C19:AA19', '{=(C15:AA15 - C16:AA16) + C16:AA16*(1-Offtaker_tax_rate/100)}', o['text_format_2'])
        baucashf_sheet.write('A20', 'Discounted Cash Flow', o['text_format_2'])
        baucashf_sheet.write_array_formula('C20:AA20', '{=C19:AA19/((1+Offtaker_discount_rate/100)^C2:AA2)}', o['text_format_2'])
        baucashf_sheet.write('A21', 'Host Net Present Value', o['standalone_bold_text_format'])
        baucashf_sheet.write_formula('B21', '=SUM(B20:AA20)', o['standalone_bold_text_format'])
    ...
    # print(results['outputs']['Financial']['lcc'])
    # print(get_value('outputs','Financial','lcc_bau'))
    # print('row = ' + str(row))
