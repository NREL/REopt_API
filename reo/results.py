...
def parse_run_outputs(self, dfm_list, data, meta, saveToDB=True):
    ...
    try:
        ...
        pv_watts_station_check = data['outputs']['Scenario']['Site']['PV'].get('station_distance_km',0)
        if pv_watts_station_check > 322:
            pv_warning = "The best available solar resource data is more than 200 miles ({} miles) from the site's coordinates. Consider choosing an alternative location closer to the continental US with similar solar irradiance and weather patterns and rerunning the analysis".format(pv_watts_station_check*0.621)
