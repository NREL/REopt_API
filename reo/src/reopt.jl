...
function reopt(reo_model, data, model_inputs)
    ...
    p = build_param(Tech = model_inputs["Tech"],
		  ...
		  TimeStepsWithGrid = model_inputs["TimeStepsWithGrid"],
          TimeStepsWithoutGrid = model_inputs["TimeStepsWithoutGrid"],
          ...
          FuelBurnYInt = model_inputs["FuelBurnYInt"],
          ...
          PricingTierCount = model_inputs["PricingTierCount"],
          ElecStorage = model_inputs["ElecStorage"],
          FuelTypeByTech = model_inputs["FuelTypeByTech"],
          SubdivisionByTech = model_inputs["SubdivisionByTech"],
          SegByTechSubdivision = model_inputs["SegByTechSubdivision"],
          TechsChargingStorage = model_inputs["TechsChargingStorage"],
          TechsInClass = model_inputs["TechsInClass"],
          TechsByFuelType = model_inputs["TechsByFuelType"],
          ElectricTechs = model_inputs["ElectricTechs"],
          FuelBurningTechs = model_inputs["FuelBurningTechs"],
          TechsNoTurndown = model_inputs["TechsNoTurndown"],
          SalesTiers = model_inputs["SalesTiers"],
          StorageSalesTiers = model_inputs["StorageSalesTiers"],
          NonStorageSalesTiers = model_inputs["NonStorageSalesTiers"]
    )
...
function reopt_run(reo_model, MAXTIME::Int64, p::Parameter)
    ...
    TotalGenFuelCharges = p.pwf_e * p.TimeStepScaling *  sum(sum(value(dvFuelUsage[t,ts]) for t in TempTechByFuelType[f], ts in p.TimeStep) * p.FuelCost[f] for f in p.FuelType))
