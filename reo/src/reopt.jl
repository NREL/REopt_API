...
function reopt_run(p::Parameter)
    ...
    REopt = direct_model(Xpress.Optimizer())
	...
	if !isempty(PVTechs)
		...
		@expression(REopt, PVPerUnitSizeOMCosts,
					sum(OMperUnitSize[t] * pwf_om * dvSystemSize[t, s] for t in PVTechs, s in p.Seg))
