...
function add_RE_and_emissions_constraints(m,p) 
	...
	if p.REAccountingMethod == 0 # no RE "credit" for exported RE
		...
		@constraint(m, MinRECon, m[:AnnualREkWh_NoExportRECredit] >= p.MinAnnualPercentRE*p.AnnualElecLoad)
		@constraint(m, MaxRECon, m[:AnnualREkWh_NoExportRECredit] <= p.MaxAnnualPercentRE*p.AnnualElecLoad)
	elseif p.REAccountingMethod == 1 # yes RE "credit" for exported RE
		m[:AnnualREkWh] = m[:AnnualREkWh_YesExportRECredit]
		@constraint(m, MinRECon, m[:AnnualREkWh_YesExportRECredit] >= p.MinAnnualPercentRE*p.AnnualElecLoad)
		@constraint(m, MaxRECon, m[:AnnualREkWh_YesExportRECredit] <= p.MaxAnnualPercentRE*p.AnnualElecLoad)
	end
